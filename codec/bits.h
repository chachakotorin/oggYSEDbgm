...
static __inline uint32_t getdword(void *mem)
{
    uint32_t tmp;
...
static /*__inline*/ uint32_t getdword_n(void *mem, int n)
{
    uint32_t tmp = 0;
#ifndef ARCH_IS_BIG_ENDIAN
    switch (n)
...
static __inline uint32_t faad_showbits(bitfile *ld, uint32_t bits)
{
...
static __inline void faad_flushbits(bitfile *ld, uint32_t bits)
{
    /* do nothing if error */
    if (ld->error != 0)
        return;
...
static /*__inline*/ uint32_t faad_getbits(bitfile *ld, uint32_t n DEBUGDEC)
{
    uint32_t ret;
...
static __inline uint8_t faad_get1bit(bitfile *ld DEBUGDEC)
{
    uint8_t r;
...
static __inline uint32_t faad_showbits_rev(bitfile *ld, uint32_t bits)
{
    uint8_t i;
    uint32_t B = 0;
...
static __inline void faad_flushbits_rev(bitfile *ld, uint32_t bits)
{
    /* do nothing if error */
    if (ld->error != 0)
        return;
...
static /*__inline*/ uint32_t faad_getbits_rev(bitfile *ld, uint32_t n
                                        DEBUGDEC)
{
    uint32_t ret;
...
static __inline uint32_t showbits_hcr(bits_t *ld, uint8_t bits)
{
    if (bits == 0) return 0;
    if (ld->len <= 32)
    {
...
static __inline int8_t flushbits_hcr( bits_t *ld, uint8_t bits)
{
    ld->len -= bits;
...
static __inline int8_t getbits_hcr(bits_t *ld, uint8_t n, uint32_t *result)
{
    *result = showbits_hcr(ld, n);
    return flushbits_hcr(ld, n);
}

static __inline int8_t get1bit_hcr(bits_t *ld, uint8_t *result)
{
    uint32_t res;
    int8_t ret;
