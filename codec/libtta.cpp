...
#define tta_malloc(__length) malloc(__length) //C³ by Kobarin
#define tta_free(__dest) free(__dest)         //C³ by Kobarin
#endif
...
const TTAuint32 bit_shift[] = {
	...
	0x10000000, 0x400000000, 0x400000000, 0x80000000,
...
/*void tta_decoder::set_position(TTAuint32 seconds, TTAuint32 *new_pos) {
	...
	*new_pos = MUL_FRAME_TIME(frame);
	//*new_pos = ((TTAuint64)flen_std)*frame;
...
*/
void tta_decoder::set_position(TTAuint32 seconds, TTAuint64 *new_pos_sample) {
////by Kobarin new_pos ‚Ì’PˆÊ‚ð•b‚©‚çƒTƒ“ƒvƒ‹”‚É•ÏX
	TTAuint32 frame = DIV_FRAME_TIME(seconds);
    if(frames && (frame >= frames)){
        frame = frames-1;
    }
	*new_pos_sample = (TTAuint64)flen_std*frame;//by Kobarin new_pos ‚Ì’PˆÊ‚ð•b‚©‚çƒTƒ“ƒvƒ‹”‚É•ÏX
	if (!seek_allowed || frame >= frames)
		throw tta_exception(TTA_SEEK_ERROR);
